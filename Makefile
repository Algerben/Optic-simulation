...
# Set global macros
buildDir := bin
executable := $(APPNAME)# modified so I can set the app name from tasks.json
target := $(buildDir)/$(executable)
sources := $(call rwildcard,src/,*.cpp)
objects := $(patsubst src/%, $(buildDir)/%, $(patsubst %.cpp, %.o, $(sources)))
depends := $(patsubst %.o, %.d, $(objects))
compileFlags := -std=c++14 -I include
compileFlags += $(COMPILEARGS)# modified so I can pass compiler args
linkFlags = -L lib/$(platform) -l raylib
linkFlags += $(EXLINKFLAGS)# modified so I can set the link flags from tasks.json
